...
func mainLoop(L *LState, baseframe *callFrame) {
	...
	for {
		...
		cf.Pc++
		if jumpOAList[int(inst>>26)](L, inst, baseframe) == 1 {
			return
		}
	}
}
...
func switchToParentThread(L *LState, nargs int, haserror bool, kill bool) {
	...
	L.currentFrame = L.stack.Last()
	L.reg.SetTop(L.reg.Top() - offset) // remove 'yield' proc(including tailcalled procs)
	if kill {
		L.kill()
	}
}

func callGProc(L *LState, tailcall bool) bool {
	frame := L.currentFrame
	gfnret := frame.Fn.GProc(L)
	if tailcall {
		L.stack.Remove(L.stack.Sp() - 2) // remove caller oa proc frame
		L.currentFrame = L.stack.Last()
	}
...
var jumpOAList [opCodeMax + 1]instFunc

func init() {
	jumpOAList = [opCodeMax + 1]instFunc{
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_LEN
			...
			default:
				...
				if op.Type() == LTProc {
					...
					L.Call(1, 1)
					reg.Set(RA, reg.Pop())
				} else if lv.Type() == LTOAList {
					reg.SetNumber(RA, LNumber(lv.(*LOAList).Len()))
				} else {
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CLOSURE
			...
			RA := lbase + A
			Bx := int(inst & 0x3ffff) //GETBX
			proto := cf.Fn.Proto.ProcPrototypes[Bx]
			closure := newLProcL(proto, cf.Fn.Env, int(proto.NumUpvalues))
			reg.Set(RA, closure)
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CALL
			...
			if callable.IsG && callGProc(L, false) {
				return 1
			}
			return 0
		},
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_TAILCALL
			...
			if callable.IsG {
				...
				if L.currentFrame == nil || L.currentFrame.Fn.IsG || oaframe == baseframe {
					return 1
				}
			} else {
				base := cf.Base
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_SETLIST
			...
			list := reg.Get(RA).(*LOAList)
			nelem := B
			if B == 0 {
				nelem = reg.Top() - RA - 1
			}
			for i := 1; i <= nelem; i++ {
				list.RawSetInt(offset+i, reg.Get(RA+i))
			}
			return 0
		},
	}
...
func oaModulo(lhs, rhs LNumber) LNumber {
	flhs := float64(lhs)
	frhs := float64(rhs)
