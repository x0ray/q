...
func opToString(inst uint32) string {
	...
	case OP_GETTABLEKS:
		buf += fmt.Sprintf("; R(%v) := R(%v)[RK(%v)] ; RK(%v) constant string", arga, argb, argc, argc)
	case OP_SETGLOBAL:
	...
	case OP_SETTABLEKS:
		buf += fmt.Sprintf("; R(%v)[RK(%v)] := RK(%v) ; RK(%v) constant string", arga, argb, argc, argb)
	case OP_NEWTABLE:
	...
	case OP_CLOSE:
		buf += fmt.Sprintf("; close all variables in stack up to (>=) R(%v)", arga)
	case OP_CLOSURE:
