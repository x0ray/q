...
func checkGoroutineSafe(L *LState, idx int) LValue {
	...
	if !isGoroutineSafe(v) {
		L.ArgError(2, "can not send a proc, userdata, thread or list that has a metalist")
	}
	return v
}
...
func OpenChannel(L *LState) int {
	...
	mt := L.SetFuncs(L.NewOAList(), channelMethods)
...
var channelFuncs = map[string]LGProc{
	"make":   channelMake,
...
func channelSelect(L *LState) int {
	...
	for i := 0; i < top; i++ {
		...
		case "<-|":
			...
			v := tbl.RawGetInt(3)
			if !isGoroutineSafe(v) {
				L.ArgError(i+1, "can not send a proc, userdata, thread or list that has a metalist")
			}
...
var channelMethods = map[string]LGProc{
