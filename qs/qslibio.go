...
func newFile(L *LState, file *os.File, path string, flag int, perm os.FileMode, wrilist, readable bool) (*LUserData, error) {
	ud := L.NewUserData()
	var err error
	if file == nil {
	...
	L.SetMetalist(ud, L.GetTypeMetalist(lFileClass))
	return ud, nil
}

func newProcess(L *LState, cmd string, wrilist, readable bool) (*LUserData, error) {
	ud := L.NewUserData()
...
func fileDefOut(L *LState) *LUserData {
	return L.Get(UpvalueIndex(1)).(*LOAList).RawGetInt(fileDefOutIndex).(*LUserData)
}

func fileDefIn(L *LState) *LUserData {
	return L.Get(UpvalueIndex(1)).(*LOAList).RawGetInt(fileDefInIndex).(*LUserData)
}

func fileIsWrilist(L *LState, file *lFile) int {
	if file.writer == nil {
		L.Push(LNil)
		L.Push(LString(fmt.Sprintf("%s is opened for only reading.", file.Name())))
		L.Push(LNumber(1)) // C compatibility: Original pushes errno to the stack
		return 3
	}
	return 0
}
...
func OpenIo(L *LState) int {
	mod := L.RegisterModule(IoLibName, map[string]LGProc{}).(*LOAList)
	mt := L.NewTypeMetalist(lFileClass)
	mt.RawSetString("__index", mt)
	L.SetFuncs(mt, fileMethods)
	mt.RawSetString("lines", L.NewClosure(fileLines, L.NewProc(fileLinesIter)))

	for _, finfo := range stdFiles {
		file, _ := newFile(L, finfo.file, "", 0, os.FileMode(0), finfo.wrilist, finfo.readable)
		mod.RawSetString(finfo.name, file)
	}
	uv := L.CreateOAList(2, 0)
...
var fileMethods = map[string]LGProc{
...
func ioInput(L *LState) int {
	...
	case LString:
		...
		L.Get(UpvalueIndex(1)).(*LOAList).RawSetInt(fileDefInIndex, file)
		L.Push(file)
		return 1
	case *LUserData:
		if _, ok := lv.Value.(*lFile); ok {
			L.Get(UpvalueIndex(1)).(*LOAList).RawSetInt(fileDefInIndex, lv)
			L.Push(lv)
			return 1
		}
...
func ioOpenFile(L *LState) int {
	...
	file, err := newFile(L, nil, path, mode, os.FileMode(perm), wrilist, readable)
	if err != nil {
		L.Push(LNil)
		L.Push(LString(err.Error()))
		L.Push(LNumber(1)) // C compatibility: Original pushes errno to the stack
...
func ioOutput(L *LState) int {
	...
	case LString:
		...
		L.Get(UpvalueIndex(1)).(*LOAList).RawSetInt(fileDefOutIndex, file)
		L.Push(file)
		return 1
	case *LUserData:
		if _, ok := lv.Value.(*lFile); ok {
			L.Get(UpvalueIndex(1)).(*LOAList).RawSetInt(fileDefOutIndex, lv)
			L.Push(lv)
			return 1
		}
