...
func osStat(L *LState) int {
	...
	t = time.Unix(b.Atimespec.Unix())
	ret.RawSetString("atim", LString(strftime(t, "%c")))
	t = time.Unix(b.Mtimespec.Unix())
	ret.RawSetString("mtim", LString(strftime(t, "%c")))
	t = time.Unix(b.Ctimespec.Unix())
...
func osStatfs(L *LState) int {
	...
	ret.RawSetString("fsid1", LNumber(b.Fsid.Val[0]))
	ret.RawSetString("fsid2", LNumber(b.Fsid.Val[1]))
	ret.RawSetString("namelen", LNumber(len(path)))
	ret.RawSetString("frsize", LNumber(0)) // Does fragment size apply to MacOS?
	ret.RawSetString("typename", LString(fileSystemType(int64(b.Type))))
