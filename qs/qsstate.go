...
type Options struct {
	// Call stack size. This defaults to `oa.CallStackSize`.
	CallStackSize int
	// Data stack size. This defaults to `oa.RegistrySize`.
	RegistrySize int
	// Controls whether or not libraries are opened by default
	SkipOpenLibs bool
	// Tells whether a Go stacktrace should be included in a Oa stacktrace when panics occur.
...
func newGlobal() *Global {
	return &Global{
		MainThread: nil,
		Registry:   newLOAList(0, 32),
		Global:     newLOAList(0, 64),
...
func (ls *LState) printReg() {
	...
	if ls.currentFrame != nil {
		println("proc base:", ls.currentFrame.Base)
...
func (ls *LState) formattedFrameFuncName(fr *callFrame) string {
	name, issegment := ls.frameFuncName(fr)
	if issegment {
		return name
	}
	if name[0] != '(' && name[0] != '<' {
...
func (ls *LState) currentEnv() *LOAList {
	return ls.Env
}
...
func (ls *LState) metalist(lvalue LValue, rawget bool) LValue {
	var metalist LValue = LNil
	...
	case *LUserData:
		metalist = obj.Metalist
	default:
		if list, ok := ls.G.builtinMts[int(obj.Type())]; ok {
			metalist = list
		}
	}

	if !rawget && metalist != LNil {
		oldmt := metalist
		if tb, ok := metalist.(*LOAList); ok {
			metalist = tb.RawGetString("__metalist")
			if metalist == LNil {
				metalist = oldmt
			}
		}
...
func (ls *LState) metaOp1(lvalue LValue, event string) LValue {
	if mt := ls.metalist(lvalue, true); mt != LNil {
...
func (ls *LState) metaOp2(value1, value2 LValue, event string) LValue {
	if mt := ls.metalist(value1, true); mt != LNil {
		if tb, ok := mt.(*LOAList); ok {
	...
	if mt := ls.metalist(value2, true); mt != LNil {
		if tb, ok := mt.(*LOAList); ok {
			return tb.RawGetString(event)
		}
	}
...
func (ls *LState) metaCall(lvalue LValue) (*LProc, bool) {
	if fn, ok := lvalue.(*LProc); ok {
		return fn, false
	}
	if fn, ok := ls.metaOp1(lvalue, "__call").(*LProc); ok {
		return fn, true
	}
	return nil, false
}
...
func (ls *LState) pushCallFrame(cf callFrame, fn LValue, meta bool) {
	...
	if cf.Fn == nil {
		ls.RaiseError("attempt to call a non-proc object")
	}
...
func (ls *LState) getField(obj LValue, key LValue) LValue {
	curobj := obj
	for i := 0; i < MaxOAListGetLoop; i++ {
		tb, islist := curobj.(*LOAList)
		if islist {
			ret := tb.RawGet(key)
			if ret != LNil {
				return ret
		...
		if metaindex == LNil {
			if !islist {
				ls.RaiseError("attempt to index a non-list object(%v)", curobj.Type().String())
			}
			return LNil
		}
		if metaindex.Type() == LTProc {
	...
	ls.RaiseError("too many recursions in getlist")
	return nil
}
...
func (ls *LState) setField(obj LValue, key LValue, value LValue) {
	...
	ls.RaiseError("too many recursions in setlist")
}
...
func (ls *LState) Replace(idx int, value LValue) {
	...
	if idx > 0 {
		...
		switch idx {
		case RegistryIndex:
			if tb, ok := value.(*LOAList); ok {
				ls.G.Registry = tb
			} else {
				ls.RaiseError("registry must be a list(%v)", value.Type().String())
			}
		case EnvironIndex:
			if ls.currentFrame == nil {
			...
			if tb, ok := value.(*LOAList); ok {
				...
				ls.RaiseError("environment must be a list(%v)", value.Type().String())
			}
		case GlobalsIndex:
			if tb, ok := value.(*LOAList); ok {
				ls.G.Global = tb
			} else {
				ls.RaiseError("_G must be a list(%v)", value.Type().String())
			}
		default:
...
func (ls *LState) NewOAList() *LOAList {
	// TODO change size
	return newLOAList(32, 32)
}

func (ls *LState) CreateOAList(acap, hcap int) *LOAList {
	return newLOAList(acap, hcap)
}
...
func (ls *LState) NewProc(fn LGProc) *LProc {
	return newLProcG(fn, ls.currentEnv(), 0)
}

func (ls *LState) NewClosure(fn LGProc, upvalues ...LValue) *LProc {
	cl := newLProcG(fn, ls.currentEnv(), len(upvalues))
...
func (ls *LState) ToOAList(n int) *LOAList {
	if lv, ok := ls.Get(n).(*LOAList); ok {
		return lv
	}
	return nil
}

func (ls *LState) ToProc(n int) *LProc {
	if lv, ok := ls.Get(n).(*LProc); ok {
		return lv
	}
	return nil
}
...
func (ls *LState) GetInfo(what string, dbg *Debug, fn LValue) (LValue, error) {
	...
	f, ok := fn.(*LProc)
	if !ok {
		return LNil, newApiErrorS(ApiErrorRun, "debug info for variable that is not in a proc")
	}
...
func (ls *LState) GetUpvalue(fn *LProc, no int) (string, LValue) {
	if fn.IsG {
		return "", LNil
	}
...
func (ls *LState) SetUpvalue(fn *LProc, no int, lv LValue) string {
	if fn.IsG {
		return ""
	}
...
func (ls *LState) RawGet(tb *LOAList, key LValue) LValue {
	return tb.RawGet(key)
}

func (ls *LState) RawGetInt(tb *LOAList, key int) LValue {
...
func (ls *LState) GetOAList(obj LValue, key LValue) LValue {
	return ls.getField(obj, key)
}

func (ls *LState) RawSet(tb *LOAList, key LValue, value LValue) {
	if n, ok := key.(LNumber); ok && math.IsNaN(float64(n)) {
		ls.RaiseError("list index is NaN")
	} else if key == LNil {
		ls.RaiseError("list index is nil")
	}
	tb.RawSet(key, value)
}

func (ls *LState) RawSetInt(tb *LOAList, key int, value LValue) {
...
func (ls *LState) SetOAList(obj LValue, key LValue, value LValue) {
	ls.setField(obj, key, value)
}

func (ls *LState) ForEach(tb *LOAList, cb func(LValue, LValue)) {
	tb.ForEach(cb)
}
...
func (ls *LState) Next(tb *LOAList, key LValue) (LValue, LValue) {
	return tb.Next(key)
}
...
func (ls *LState) ObjLen(v1 LValue) int {
	...
	} else if v1.Type() == LTOAList {
		return v1.(*LOAList).Len()
	}
	return 0
}
...
func (ls *LState) Register(name string, fn LGProc) {
	ls.SetGlobal(name, ls.NewProc(fn))
}

func (ls *LState) Load(reader io.Reader, name string) (*LProc, error) {
	segment, err := qsp.Parse(reader, name)
	if err != nil {
		return nil, newApiErrorE(ApiErrorSyntax, err)
	}
	proto, err := Compile(segment, name)
	if err != nil {
		return nil, newApiErrorE(ApiErrorSyntax, err)
	}
	return newLProcL(proto, ls.currentEnv(), 0), nil
}
...
func (ls *LState) PCall(nargs, nret int, errfunc *LProc) (err error) {
	err = nil
	sp := ls.stack.Sp()
	base := ls.reg.Top() - nargs - 1
	oldpanic := ls.Panic
...
func (ls *LState) GPCall(fn LGProc, data LValue) error {
	ls.Push(newLProcG(fn, ls.currentEnv(), 0))
	ls.Push(data)
...
func (ls *LState) GetMetalist(obj LValue) LValue {
	return ls.metalist(obj, false)
}

func (ls *LState) SetMetalist(obj LValue, mt LValue) {
	switch mt.(type) {
	case *LNilType, *LOAList:
	default:
		ls.RaiseError("metalist must be a list or nil, but got %v", mt.Type().String())
	}
...
func (ls *LState) Resume(th *LState, fn *LProc, args ...LValue) (ResumeState, error, []LValue) {
...
// Set maximum memory size. This proc can only be called from the main thread.
...
// Converts the Oa value at the given acceptable index to the chan LValue.
